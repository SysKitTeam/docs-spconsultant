...
* Adding SPDocKit Consultant user accounts to the **Local Administrator** group on the server is needed for WMI remote access and to get information about Windows updates. In case you can’t give that kind of permissions due to security requirements, see lower for what you can grant instead this. The following reports will not be available:
...
To eliminate this warning you either have to grant the necessary permissions to SPDocKit Consultant accounts or turn off the load **Database Permissions** option.
...
If you wish to grant the necessary permissions to the SPDocKit Consultant accounts, follow this [link](../../requirements/server-load-permission-requirements.md#granting-permissions-1).
